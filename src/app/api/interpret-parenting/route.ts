...
//
// CLAUDE_ASTRO.md is NOT loaded — mechanics are inherited via CLAUDE_NATAL_PARENTING.md
// which references CLAUDE_ASTRO.md as its mechanical foundation.
// HD data is stripped from the payload (same Layer 1 as interpret/route.ts).
...
const SECTIONS: SectionDef[] = [
  ...
  { column: 'parenting_purusharthas', name: 'the Dharma, Artha, Kama, Moksha balance',                                 wordCap:  800 },
  { column: 'parenting_dasha',        name: 'the Dasha Timeline',                                                       wordCap: 4000, maxTokens: 8192 },
]

// ─── Layer 1 — Payload stripper ───────────────────────────────────────────────
//
// Strip HD data from the natal payload before sending to the parenting route.
// Mirrors the astrology route's buildAstrologyPayload exactly.
      ...
      for (const section of sectionsToRun) {
        ...
        try {
          const message = await anthropic.messages.create({
            ...
            messages: [{
              role:    'user',
              content: buildUserMessage(section, parentingPayloadStr, parentingPayload),
            }],
          })
          ...
          if (chart_id && text) {
            ...
            if (dbError) {
              console.error(`[interpret-parenting] db write failed for ${section.column}:`, dbError)
            }
          }
